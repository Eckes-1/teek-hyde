...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
使用示例:
  python script.py -d "路径1" "路径2" -p "美化"
  python script.py --directory "/path/to/docs1" "/path/to/docs2" --prefix "blog"
        """
    )
    ...
    parser.add_argument(
        ...
        nargs='+',  # 允许接收多个目录路径
        default=[
            r"D:\code\teek-hyde\docs\10.笔记专栏\02.Vdoing"
        ],  # 默认多个路径
        help='要处理的目录路径（可指定多个）'
    )
    ...
    if args.dry_run:
        for dir_path in args.directory:
            ...
            if directory.exists():
                md_files = list(directory.rglob("*.md"))
                print(f"预览模式 - 目录 {dir_path} 找到 {len(md_files)} 个.md文件:")
                for md_file in md_files:
                    print(f"  - {md_file}")
            else:
