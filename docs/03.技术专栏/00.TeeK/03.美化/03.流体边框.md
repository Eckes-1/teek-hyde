...
在 `vitepress-theme-Teek\src\styles\vp-plus` 新建 `custom-block.scss` 文件
...
最后在引入 `Demo\docs-base\.vitepress\theme\index.ts` 中 即可看到效果

```ts{2}
// 流体边框自定义容器样式  [!code focus:1]
import "vitepress-theme-teek/vp-plus/custom-block.scss";
```
