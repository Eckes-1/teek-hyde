...
::: warning 注意
`timeline`不要在“`export default defineConfig({})`进行插件注册”，否则影响`导航卡片`样式不生效。详情查看[TeeK插件配置](https://teek.seasir.top/reference/plugin-config#markdownplugins)
:::
...
```ts{1,4-6}
import timeline from "vitepress-markdown-timeline"; // 导入时间线插件 [!code focus:6]
