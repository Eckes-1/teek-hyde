...
- [茂茂的DEMO](https://fe-nav.netlify.app/nav/) 丨 [我的演示页](https://wiki.seasir.top/nav/)
...
::: tip 版权声明
本文是在 博主[yiov 中文网](https://vitepress.yiov.top/) 文章：[VitePress搭建个人导航》](https://vitepress.yiov.top/nav/) 基础上增加了自己实践过程的一些细节，转载无需和我联系，但请注明文章来源。如果侵权之处，请联系博主进行删除，谢谢~
:::
...
- 然后在`Demo\docs-base\.vitepress\config\Nav.ts` 中配置
  ::: warning 注意
  如果您没有抽离[抽离config配置](https://wiki.seasir.top/pages/b09547)，则需要在`config.mts`中配置
  :::
...
- 访问[http://localhost:5173/nav/](http://localhost:5173/nav/)查看效果
