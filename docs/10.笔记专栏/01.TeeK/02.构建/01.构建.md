...
为了解决这个问题，将使用 VitePress 提供的 `<ClientOnly>` 组件来包裹 `<Teek.Layout>` 。这样可以确保 `<Teek.Layout>` 只在客户端渲染，从而避免在构建时访问 localStorage 。

在`docs\.vitepress\theme\components\TeekLayoutProvider.vue`添加`<ClientOnly>`
